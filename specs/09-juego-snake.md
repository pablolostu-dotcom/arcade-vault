...
- **El paso 5 se cerró por el camino B: los dos efectos van sintetizados con WebAudio y no entra ningún binario al repo.** El camino A no cerró. En los bancos alcanzables no hay un par CC0 usable: Pixabay responde 403, freesound pide un token de OAuth para bajar, y en Wikimedia Commons el único efecto de fin de partida con licencia verificable —`File:Fim efeito sonoro retrô.oga`, CC0— dura 2,38 s y pesa 482 KB, veinticinco veces todo el audio del SPEC 08. Además los dos candidatos vienen en Ogg, que Safari no reproduce, y no hay `ffmpeg` en el entorno para recortarlos ni pasarlos a mp3. Comer es un blip de onda cuadrada de 880 a 1318,5 Hz en 150 ms; morir es un barrido de sierra de 440 a 55 Hz en 400 ms.
